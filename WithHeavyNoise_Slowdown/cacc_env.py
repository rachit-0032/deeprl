        ...
        for i in range(self.n_agent):
            ...
            if self.agent.startswith('ia2c'):
                for j in np.where(self.neighbor_mask[i] == 1)[0]:
                    veh_state_j = self._get_veh_state(j)
                    add_noise = np.array(np.random.laplace(NOISE_mu, NOISE_b, len(veh_state_j)))
                    with_noise = veh_state_j + add_noise
                    cur_state.append(with_noise)
